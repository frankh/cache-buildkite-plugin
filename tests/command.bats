...
@test "Pre-command restores cache with basic key" {
  ...
  assert_success
  assert_output --partial "aws s3 sync s3://my-bucket/my-org/my-pipeline/v1-cache-key/ /tmp"
...
@test "Post-command syncs artifacts" {
  ...
  assert_output --regexp "aws s3 sync .* s3://my-bucket/my-org/my-pipeline/v1-cache-key/ --delete"
